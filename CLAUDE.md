...
| `dashboard-navigation.ts`  | `NAV_GROUPS`, phases, `JOURNEY_STEPS`, team-only section set, and the pure reducers for `journey_done` (checked by `dashboard-navigation.check.ts`). |
| `journey-progress.tsx`     | The launch meter under "Your journey". Weighted in units, not steps — a step whose items are ticked one by one is worth one unit per item.           |
